        ...
        try:
            ...
            if user_profile.check_following(profile):
                formatted_response = {
                    ...
                    "message": f"You are already following {profile.username}",
                }
                return Response(formatted_response, status=status.HTTP_400_BAD_REQUEST)

            user_profile.follow(profile)
            subject = "A new user follows you"
            message = f"Hi there, {profile.username}!!, the user {user_profile.username} now follows you"
...
class UnfollowAPIView(APIView):
    def post(self, request, user_id, *args, **kwargs):
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                "status_code": status.HTTP_400_BAD_REQUEST,
                "message": f"You can't unfollow {profile.username}, since you were not following them in the first place ",
            }
            return Response(
                formatted_response,
