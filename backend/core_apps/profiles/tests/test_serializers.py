...
class ProfileSerializerTest(TestCase):
    ...
    def test_contains_expected_fields(self):
        ...
        expected_fields = set(self.serializer_data.keys()).union({'profile_photo', 'full_name', 'id'})
    ...
    def test_profile_deserialization(self):
        ...
        if 'username' in serializer_data:
            self.assertEqual(serializer.validated_data['username'], serializer_data['username'])
        if 'first_name' in serializer_data:
            self.assertEqual(serializer.validated_data['first_name'], serializer_data['first_name'])
        if 'last_name' in serializer_data:
            self.assertEqual(serializer.validated_data['last_name'], serializer_data['last_name'])
        self.assertEqual(serializer.validated_data['phone_number'], serializer_data['phone_number'])
        self.assertEqual(serializer.validated_data['gender'], serializer_data['gender'])
        self.assertEqual(serializer.validated_data['country'], serializer_data['country'])
        self.assertEqual(serializer.validated_data['city'], serializer_data['city'])
        self.assertEqual(serializer.validated_data['twitter_handle'], serializer_data['twitter_handle'])
        self.assertEqual(serializer.validated_data['about_me'], serializer_data['about_me'])
